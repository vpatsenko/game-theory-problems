...
Applying backward induction, at time \(t\) the responder can reject the offer
and become the proposer at time \(t+1\). Hence the responder's continuation
value is
\[
V_{t+1}^P.
\]
Therefore, in equilibrium, the proposer gives the responder exactly
\[
V_{t+1}^P
\]
and keeps the rest of the current cake value. Since the value of the whole cake
at time \(t\) is \(\delta^t\), the proposer obtains
\[
