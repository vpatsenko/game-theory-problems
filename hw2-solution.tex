\textbf{2. (Parametric game \(2\times 3\)).}
Consider the following game with parameter \(a\):
\[
\begin{array}{c|ccc}
 & L & M & R\\
\hline
...
Find all pure and mixed Nash equilibriums in the cases:
\begin{enumerate}
\item[(a)] \(a=3\);
\item[(b)] \(a=6\);
\item[(c)] \(a=5\).
...
\textbf{Step 1: Best responses in general.}
...
\textbf{Step 2: Mixed equilibrium conditions.}
...
Thus the pure Nash equilibriums are
\[
(D,L),\qquad (U,R)
\]
...
\textbf{Column player uses only \(R\).}
