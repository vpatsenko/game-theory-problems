...
\item[(b)]
Find all pure and mixed equilibria in the remaining \(2\times N\) game.
\end{enumerate}

\subsection*{(a) Elimination of strictly dominated strategies}
...
\paragraph{Step 3: eliminate \(s_4\).}
...
\paragraph{Step 4: eliminate \(t_4\).}
Consider the mixture
\[
...
\paragraph{No further eliminations are possible.}
...
\subsection*{(b) Pure and mixed Nash equilibria of the remaining \(2\times 3\) game}
...
\paragraph{Row player's best responses.}
\begin{itemize}
...
\paragraph{Column player's best responses.}
\begin{itemize}
...
\paragraph{Best responses of the column player.}
We compare:
\[
...
\paragraph{A fully mixed row player.}
...
\paragraph{Mixed equilibrium with support \(\{t_1,t_2\}\).}
...
\[
\boxed{\left(\frac12 s_1+\frac12 s_3,\ \frac23 t_1+\frac13 t_2\right).}
\]

\paragraph{Equilibria with column player choosing \(t_3\).}
...
the profile
\[
\boxed{\left(ps_1+(1-p)s_3,\ t_3\right)}
\]
...
The pure Nash equilibria in this remaining game are
\[
\boxed{(s_3,t_1)\quad \text{and}\quad (s_1,t_3).}
\]
...
and the family
\[
\boxed{\left(ps_1+(1-p)s_3,\ t_3\right),\qquad p\in\left[\frac23,1\right].}
\]
